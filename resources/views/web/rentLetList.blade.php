...
<div class="rent-let">
	<div class="container">
		<div class="text-center">
			<a href="{{route('rentLetHairstylist')}}" class="btn my-4 beautyTherapistBtn {{@$page == 'hairstylist' ? 'customBtnSelected' : 'customBtn'}}">Hairstylist / Barber Chair</a>
			<br> 
			<a href="{{route('rentLetBeautyTherapist')}}" class="btn mb-4 hairstylistBtn {{@$page == 'beauty-therapist' ? 'customBtnSelected' : 'customBtn'}}">Beauty therapist Chair</a>
		</div>
					...
					<div id="24">
						<div class="row">
							<div class="col-md-6 therapist-content p-3 px-4">
								...
								<p>
									...
									@if($rentLet->rent_let_type == 'Weekly rent')
										<strong>{{@$rentLet->rent_let_type}} &pound;{{number_format(@$rentLet->weekly_rent, 2)}}</strong>
									@elseif($rentLet->rent_let_type == 'Monthly rent')
										<strong>{{@$rentLet->rent_let_type}} &pound;{{number_format(@$rentLet->monthly_rent, 2)}}</strong>
									@elseif($rentLet->rent_let_type == 'Daily rent')
										<strong>{{@$rentLet->rent_let_type}} &pound;{{number_format(@$rentLet->daily_rent, 2)}}</strong>
									@elseif($rentLet->rent_let_type == 'Hourly rent')
										<strong>{{@$rentLet->rent_let_type}} &pound;{{number_format(@$rentLet->hourly_rent, 2)}}</strong>
									@endif
							...
							<div class="col-md-6">
								...
								<div class="mt-4">
									<a class="carousel-control-prev-1" href="#carouselExampleControls_{{$rentLet->id}}" role="button" data-slide="prev">
										<span class="carousel-control-prev-icon" aria-hidden="true"></span>
										<span class="sr-only">Previous</span>
									</a>
									<a class="carousel-control-next-1" style="float: right" href="#carouselExampleControls_{{$rentLet->id}}" role="button" data-slide="next">
