...
//       //Hair Cutting
//       // let ladiesHircut = $("#stylist-customer-ladies").is(':checked') ? 1 : 0;
//       // let gentsHircut = $("#stylist-customer-gents").is(':checked') ? 1 : 0;
//       // let kidsHircut = $("#stylist-customer-kids").is(':checked') ? 1 : 0;
//       // let afroHircut = $("#stylist-customer-afro").is(':checked') ? 1 : 0;

//       //Wedding Styles
//       // let classicLowChignon = $("#classic-low-chignon").is(':checked') ? 1 : 0;
//       // let frenchChignon = $("#french-chignon").is(':checked') ? 1 : 0;
//       // let updoFlowers = $("#updo-flowers").is(':checked') ? 1 : 0;
//       // let smoothCurls = $("#smooth-curls").is(':checked') ? 1 : 0;
//       // let pinnedCurls = $("#pinned-curls").is(':checked') ? 1 : 0;
//       // let longPonytail = $("#long-ponytail").is(':checked') ? 1 : 0;
//       // let braidedBarrette = $("#braided-barrette").is(':checked') ? 1 : 0;
//       // let hiddenHairPiece = $("#hidden-hair-piece").is(':checked') ? 1 : 0;
//       // let fishtailBraid = $("#fishtail-braid").is(':checked') ? 1 : 0;
//       // let fingerWeaves = $("#finger-weaves").is(':checked') ? 1 : 0;
...
//           "_TradeTestVideoUrl":"https://www.youtube.com/",
//           "_UTR":utr,
...
function redirectToPage() {
    window.location.href = "./hairstylist-profile-buildup.html";
}

function sendEmail() {
    let ownerMessage =
        "A new Candiate registered as Hairstylist on Styzeler Website. Please go through the details on Adminpanel.";
    var settings = {
        url: "./email.php",
        method: "POST",
        timeout: 0,
...
if (typeof FileReader === "undefined") {
    result.innerHTML =
        "Sorry, your browser does not support FileReader, please use modern browsers operation!";
    gallery.setAttribute("disabled", "disabled");
} else {
    gallery.addEventListener("change", handleFiles, false);
}
...
$(function () {
    $("#register-form").submit(function (e) {
        ...
        if (stylistRate == "other") {
            if (
                $('input[name="otherRate"]').val() == 0 ||
                $('input[name="otherRate"]').val() == ""
            ) {
        ...
        weddingStyles.push(
            $("#classic-low-chignon").is(":checked") ? "Classic Low Chignon" : 0
        );
        weddingStyles.push(
            $("#french-chignon").is(":checked") ? "French Chignon" : 0
        );
        weddingStyles.push(
            $("#updo-flowers").is(":checked") ? "Updo With Flowers" : 0
        );
        weddingStyles.push(
            $("#smooth-curls").is(":checked") ? "Long, Smooth Curls" : 0
        );
        weddingStyles.push(
            $("#pinned-curls").is(":checked") ? "Pinned Curls" : 0
        );
        weddingStyles.push(
            $("#long-ponytail").is(":checked") ? "Glam Long Ponytail" : 0
        );
        weddingStyles.push(
            $("#braided-barrette").is(":checked") ? "Braided Barrette" : 0
        );
        weddingStyles.push(
            $("#hidden-hair-piece").is(":checked") ? "Hidden Hair Piece" : 0
        );
        weddingStyles.push(
            $("#fishtail-braid").is(":checked") ? "Fishtail Braid" : 0
        );
        weddingStyles.push(
            $("#finger-weaves").is(":checked") ? "Classic Finger Weaves" : 0
        );
        ...
        $.each(weddingStyles, function (i) {
            if (weddingStyles[i] != 0) {
                refinedWeddingStyles =
                    refinedWeddingStyles + weddingStyles[i] + ", ";
            }
        });
        ...
        //merging serviceName + serviceHeader +serviceSubheader
        let weddingStylesService = {
            _ServiceName: refinedWeddingStyles,
            _ServiceHeader: "Wedding Styles",
            _ServiceSubHeader: "Services",
        };
        ...
        barberMaleGroomingProducts.push(
            $("#baxter").is(":checked") ? "Baxter" : 0
        ); //
        barberMaleGroomingProducts.push(
            $("#american-crew").is(":checked") ? "American Crew" : 0
        ); //
        barberMaleGroomingProducts.push(
            $("#hanz-de-fuko-hybridized").is(":checked")
                ? "Hanz De Fuko Hybridized"
                : 0
        ); //
        barberMaleGroomingProducts.push(
        ...
        barberMaleGroomingProducts.push(
            $("#jack-dean").is(":checked") ? "Jack Dean" : 0
        ); //
        barberMaleGroomingProducts.push(
            $("#vines-vintage").is(":checked") ? "Vines Vintage" : 0
        ); //
        barberMaleGroomingProducts.push(
            $("#baxter-of-california-shave-tonic").is(":checked")
        ...
        if (lastName != "") {
            if (!lastName.match(letters)) {
                ...
                $(".terms-conditions-modal").modal("hide");
                $(".error-message").append(
                    "Surname must contain only letters!<br>"
                );
                count = 1;
            }
        } else {
        ...
        if (password != "") {
            if (!password.match(pwLetters)) {
                ...
                $(".terms-conditions-modal").modal("hide");
                $(".error-message").append(
                    "Password length must be 8-16 and password must contain atleast one number, one capital letter and one special charecter!<br>"
                );
                count = 1;
            }
        } else {
            ...
            $(".terms-conditions-modal").modal("hide");
            $(".error-message").append(
                "Please enter a password and it's length must be 8-16 and must contain atleast one number, one capital letter and one special charecter! !<br>"
            );
            count = 1;
        }
        ...
        if (resume != "") {
            if (resume.length < 20) {
                if (!resume.match(resumeLetters)) {
                    ...
                    $(".terms-conditions-modal").modal("hide");
                    $(".error-message").append(
                        "Please tell us a bit more in Resume! min - 20 letters<br>"
                    );
                    count = 1;
                }
            }
            ...
            $(".terms-conditions-modal").modal("hide");
            $(".error-message").append(
                "Tell us a bit about you and your brief working history in RESUME!<br>"
            );
            count = 1;
        }
        ...
        if (languages != "") {
            if (!languages.match(languageLetters)) {
                ...
                $(".terms-conditions-modal").modal("hide");
                $(".error-message").append(
                    "Languages must contain only letters!<br>"
                );
                count = 1;
            }
        } else {
        ...
        if (selectedZones == "") {
            ...
            $(".terms-conditions-modal").modal("hide");
            $(".error-message").append(
                "Zone of London open to work is required!<br>"
            );
            count = 1;
        }
        ...
        if (count === 0) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    services: [
                        headService,
                        weddingStylesService,
                        barberMaleGroomingService,
                        hairColorService,
                        chemicalTreatmentsService,
                        barberMaleGroomingProduct,
                        hairColorBrand,
                        chemicalTreatmentsProduct,
                    ],
                    _IsAcceptTerms: "1",
                },
                ...
                success: function (response) {
                    ...
                    if (
                        response["_ReturnMessage"] ===
                        "Please enter your profile image id"
                    ) {
                        $(".error-modal").modal("show");
                        $(".terms-conditions-modal").modal("hide");
                        $(".error-message").append(
                    ...
                    //Age validation
                    if (
                        response["_ReturnMessage"] === "Please enter your age"
                    ) {
                        $(".error-modal").modal("show");
                        $(".terms-conditions-modal").modal("hide");
                    ...
                    //UTR Validation
                    if (
                        response["_ReturnMessage"] === "Please enter your utr"
                    ) {
                        $(".error-modal").modal("show");
                        $(".terms-conditions-modal").modal("hide");
                        $(".error-message").append(
                    ...
                    //Language Validation
                    if (
                        response["_ReturnMessage"] === "Please enter languages"
                    ) {
                        $(".error-modal").modal("show");
                        $(".terms-conditions-modal").modal("hide");
