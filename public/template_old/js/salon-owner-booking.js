...
function confirmAppointment(emailId, id, confirmedDate) {
    ...
    let ownerMessage =
        "BUSINESS OWNER HAS CONFIRMED THE ON HOLD BOOKING FOR " + AppntDate; //Updated by Rumki
    // _AppointmentDate
        ...
        success: function (response) {
            ...
            if (response.appointments.length === 0) {
                ...
                $.each(response.appointments, function (i) {
                    if (
                        response.appointments[i]["_Status"] !== "cancel" &&
                        response.appointments[i]["_Status"] !== "CANCELLED" &&
                        response.appointments[i]["_Status"] !==
                            "CANCELLED by Salon Owner" &&
                        response.appointments[i]["_Status"] !==
                            "cancel by Freelancer" &&
                        response.appointments[i]["_Status"] !==
                            "CANCELLED due to Expired Time"
                    ) {
                        let id = response.appointments[i]["_AppointmentId"];
                        let appDate =
                            response.appointments[i]["_AppointmentDate"];
                        let emailId =
                            response.appointments[i]["_FreelancerEmail"];
                        // console.log(appDate);
                        ...
                        $(".appointment-row").append(
                            '<div class="col-4">' +
                                ...
                                response.appointments[i]["_FreelancerName"] +
                                "</p>" +
                                "</div>" +
                                ...
                                // '</div>' +
                                '<div class="text-center customBtn " id="cancelBtn' +
                                id +
                        ...
                        if (
                            ...
                            response.appointments[i]["_Status"] === "On Hold"
                        ) {
                            $("#show" + i).append(
                                '<div class="text-center customBtn confirm_btn' +
                                    i +
                                    '" >' +
                                    "<a onclick=\"confirmAppointment('" +
                                    emailId +
                                    "' , " +
                                    id +
                                    ",'" +
                        ...
                        if (
                            response.appointments[i]["availableDays"].length > 0
                        ) {
                            $("#show" + i).append(
                                '<div class="text-center customBtn confirm_btn' +
                                    i +
                        ...
                        if (
                            ...
                            response.appointments[i]["_Status"] ===
                                "confirmed" ||
                            response.appointments[i]["_Status"] === "Booked" ||
                            response.appointments[i]["_Status"] === "booked"
                        ) {
                            hideCancelBtn(appDate, id);
                        }
                        ...
                        $.each(
                            response.appointments[i]["availableDays"],
                            function (d) {
                                $("#show-dates" + i).append(
                                    "<br>" +
                                        '<div class="form-check form-check-inline"> ' +
                                        '<input class="form-check-input" type="radio" name="avaliable_date" value= "' +
                                        response.appointments[i][
                                            "availableDays"
                                        ][d]["freelancer_available_date"] +
                                        '" > ' +
