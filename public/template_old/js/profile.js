                ...
                if (response["FreelancerReview"].length > 0) {
                    ...
                    $.each(response["FreelancerReview"], function (i) {
                        if (response["FreelancerReview"][i]["_Review"] != "") {
                            $("#showReviewLike").append(
                                "<div>" +
                                    '<h5 class="color-1">' +
                                    response["FreelancerReview"][i][
                                        "_SalonOwnerBusinessName"
                                    ] +
                                    "</h5>" +
                                    '<p class="mt-2">' +
                                    response["FreelancerReview"][i]["_Review"] +
                                    "</p>" +
                                    "</div>"
                            );
                        }
        ...
        success: function (response) {
            ...
            $.each(response.freelancers, function (i) {
                if (response.freelancers[i]["_StId"] === stId) {
                    ...
                    $("#ownerAge").text(
                        response.freelancers[i]["_Age"] + " years" + " "
                    );
                    $("#ownerAge").append(
                        '<a class="text-center btn"  onclick="editAge()" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    $("#ownerResume").text(
                        response.freelancers[i]["_Resume"] + " "
                    );
                    $("#ownerResume").append(
                        '<a class="text-center btn"  onclick="editResume()" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    $("#ownerRate").text(
                        response.freelancers[i]["_Rate"] + " "
                    );
                    $("#ownerRate").append(
                        '<a class="text-center btn"  onclick="editRate()" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    $("#ownerWork").text(
                        response.freelancers[i]["_Work"] + " "
                    );
                    $("#ownerWork").append(
                        '<a class="text-center btn"  onclick="editWork()" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    $("#ownerLanguage").text(
                        response.freelancers[i]["_Languages"] + " "
                    );
                    $("#ownerLanguage").append(
                        '<a class="text-center btn"  onclick="editLanguage()" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    ...
                    $("#status").append(
                        '<a class="text-center btn"  onclick="editStatus();" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    $("#profiletype").text(response.freelancers[i]["_Ptype"]);
                    $("#profiletype").append(
                        '<a class="text-center btn"  onclick="editType();" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    ...
                    $("#ownerQualification").append(
                        '<a class="text-center btn"  onclick="editQualification()" title="Edit">' +
                            "✎" +
                            "</a>"
                    );
                    ...
                    $.each(response.freelancers[i].images, function (j) {
                        $("#gallery-content").append(
                            '<div class="col-lg-4 p-4">' +
                                '<img id="gallery-image-id-' +
                                j +
                                '" alt="" width="100%" height="100%">' +
                                '<p><a class="text-center btn"  onclick="deletePicture(' +
                                response.freelancers[i].images[j][
                                    "gallery_image_id"
                                ] +
                                ')" title="Edit">' +
                                "<u> remove </u>" +
                        ...
                        $.ajax({
                            ...
                            success: function (galleryResponse) {
                                ...
                                $("#gallery-image-id-" + j).attr(
                                    "src",
                                    "data:image;base64," +
                                        galleryResponse["_ImageContent"]
                                );
                            },
                        });
                    });
...
function editServiceAndProduct() {
    if (
        registrationMode === "Wedding Stylist" ||
...
function updateProfile() {
    ...
    if (registrationMode === "Beautician") {
        ...
        $.each(saveService, function (service) {
            ...
            if (
                saveService[service]["service_sub_header"] === "Mani Pedi" &&
                saveService[service]["service_header"] === "Services"
            ) {
                if (refinedmaniPedi != "") {
