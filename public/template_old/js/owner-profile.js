...
let firstName, lastName, mobile, age, profileImageId, resume, nvq2, nvq3, nvqAssesor,
    ...
    success: function(response) {
        ...
        $.each ( response.freelancers, function(i) {
            ...
            if(response.freelancers[i]["_StId"] == ownerStId) {
              ...
              localStorage.setItem(ISACTIVE, response.freelancers[i]["_IsActive"]);
              ...
              console.log(saveService);
                ...
                $("#owner-name").append('<a class="text-center btn" onclick="editName()" title="Edit">' + "✎" + "</a>");
                ...
                $.each(response.freelancers[i].services, function(service){
                    ...
                    if(response.freelancers[i].services[service]["service_name"] != "") {
                        ...
                        if(response.freelancers[i].services[service]["service_header"] === "Services") {
                            ...
                            $("#ownerProducts").append('<br><div class="text-left"><p>'+
                            response.freelancers[i].services[service]["service_sub_header"]+ '</p>'+
                            '<div>' +
                            '<p><strong>'+response.freelancers[i].services[service]["service_header"]+
                            ':  </strong>' + response.freelancers[i].services[service]["service_name"] .substring(0, response.freelancers[i].services[service]["service_name"].length-2 ) + '.'  + '</p>' +
...
function updateProfile() {
  ...
  $.each(saveService, function(service){
    ...
    if(saveService[service]["service_sub_header"] === "Chemical Treatment" && saveService[service]["service_header"] === "Services" ) {
...
$.each(saveService, function(service){
  ...
  if(saveService[service]["service_sub_header"] === "Body Waxing" && saveService[service]["service_header"] === "Services" ) {
    if(refinedBodyWax != "") {
  ...
  if(saveService[service]["service_sub_header"] === "Manicure Pedicure" && saveService[service]["service_header"] === "Services" ) {
    if(refinedmaniPedi != "") {
      saveService[service]["service_name"] = refinedmaniPedi;
    }
  }
  if(saveService[service]["service_sub_header"] === "Manicure Pedicure" && saveService[service]["service_header"] === "Products" ) {
  ...
  console.log(ownerStId, registrationMode, registrationEmail, firstName, lastName, mobile, age, profileImageId, resume, nvq2, nvq3, nvqAssesor,
    tradeTestVideo, utr, publicLiabilityImage, rate, language, address, postcode, imageId, saveService);
