$(window).on("load", function () {
	...
	function serviceWidth() {
		...
		if (w > 767) {
			...
			$("#services .text_list .btns .shadow_btn").each(function () {
				$(this).outerWidth(final_svc_btn_width + "rem")
			})
	...
	function header() {
		...
		if (w <= 991) {
			...
			$("header").prepend("<div class='logo'>" + header + "</div><button type='button' class='toggle'><span></span></button>")
		}
	}
	header()
	function services() {
		let w = $(window).width()
		if (w <= 767) {
			$("#services .text_list .txt_wrap, #services .text_list").prepend("<button type='button' class='back_button'>Back</button>")

			$(document).on("click", "#services .btn_list .shadow_btn", function () {
				$("#services .text_list").addClass("active")
			})
			$(document).on("click", "#services .text_list > .back_button", function () {
				$("#services .text_list").removeClass("active")
			})
			$(document).on("click", "#services .text_list .sub_btns .shadow_btn", function () {
				$("#services .text_list").addClass("flow")
				$("#services .text_list .txt_wrap").addClass("active")
			})
			$(document).on("click", "#services .text_list .txt_wrap > .back_button", function () {
