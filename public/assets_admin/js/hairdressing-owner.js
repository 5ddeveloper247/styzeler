...
function redirect() {
    window.location.href = "./hairdressing-owner.html";
}
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each( response.freelancers, function(i){
                ...
                // $(".user-details").append('<div class="col-3 pt-2" style="border-top: 1px solid grey;">' +
                //   '<p>' + response.freelancers[i]['_RegistrationMode'] + '</p>' +
                //   '<p><img id="image_id_"' + i +'"/>' + '</p>' +
                //    '</div>' +
                //     '<div class="col-3 pt-2" style="border-top: 1px solid grey;">' +
                //         '<p><strong>Name: </strong>' + response.freelancers[i]['_FirstName'] + " " + response.freelancers[i]['_LastName'] + '</p>' +
                //         '<p><strong>Address: </strong>' + response.freelancers[i]['_Address'] + '</p>' +
                //         '<p><strong>Postcode: </strong>' + response.freelancers[i]['_PostCode'] + '</p>' +
                //         '<p><strong>Mobile Number: </strong>' + response.freelancers[i]['_MobileNumber'] + '</p>' +
                //         '<p><strong>Email: </strong>' + response.freelancers[i]['_EmailId'] + '</p>' +
                //     '</div>' + 
                //     '<div class="col-3 pt-2" style="border-top: 1px solid grey;">' +
                ...
                //     '</div>' +
                //     '<div class="col-3 pt-2" style="border-top: 1px solid grey;">' +
                //         '<p id="' + response.freelancers[i]['_StId'] +'" onclick="activePeople(' + response.freelancers[i]['_StId'] + ');">Active</p>' +
                // '</div><hr/>');
