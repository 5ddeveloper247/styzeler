...
function redirectToPage() {
    // location.reload();
    window.location.href = "beauty-chair.html";
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each( response.salonSpaces, function(i){
                ...
                // $(".chair-details").append('<div class="col-3 pt-2" style="border-top: 1px solid grey;">' +
                //   '<p>' + response.salonSpaces[i]['_Category'] + '</p>' +
                //   '<p>' + response.salonSpaces[i]['_SpaceDescription'] + '</p>' + 
                //    '</div>' +
                //     '<div class="col-3 pt-2" style="border-top: 1px solid grey;">' +
                //         '<p><strong>Salon Name:</strong>' + response.salonSpaces[i]['_SalonName'] +'</p>' +
                //         '<p><strong>Salon Address:</strong>' + response.salonSpaces[i]['_SalonAddress'] +'</p>' +
                //         '<p><strong>Salon Country:</strong>' + response.salonSpaces[i]['_SalonCountry'] + '</p>' +
                //         '<p><strong>Salon Postal Code:</strong>' + response.salonSpaces[i]['_SalonPostalCode'] + '</p>' +
                //         '<p><strong>Salon Mobile Number:</strong>' + response.salonSpaces[i]['_SalonMobileNumber'] + '</p>' +
                //         '<p><strong>Salon Email Id:</strong>' + response.salonSpaces[i]['_SalonEmailId'] + '</p>' +
                //     '</div>' + 
                //     '<div class="col-3 pt-2" style="border-top: 1px solid grey;">' +
                //        '<p>Images</p>' +
                //     '</div>' +
